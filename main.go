...
func main() {
	...
	hostname, err := rcService.Projects.Configs.Variables.Get(fmt.Sprintf("projects/%s/configs/prod/variables/hostname", os.Getenv("GOOGLE_CLOUD_PROJECT"))).Do()
	if err != nil {
		log.Fatalf("Failed to get bot user ID: %s", err)
	}
	oauth1Config := &oauth1.Config{
		...
		CallbackURL:    fmt.Sprintf("https://%s/oauth_callback", hostname),
